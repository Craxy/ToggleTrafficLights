...
{
  internal sealed class DebugLevel : IDisposable
  {
    ...
    private void ShowStatus(Level level)
    {
      using (Layout.Vertical())
      {
        var im = InfoManager.instance;
        GUILayout.Label($"State: {level.Current}");
        GUILayout.Label($"Tool: {level.CurrentTool?.GetType().Name}");
    ...
    private void ShowMessages(Level level)
    {
      using (Layout.Vertical())
      {
    ...
    private void ShowLog(Level level)
    {
