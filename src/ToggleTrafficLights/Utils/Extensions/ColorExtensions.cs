...
namespace Craxy.CitiesSkylines.ToggleTrafficLights.Utils.Extensions
{
    public static class ColorExtensions
    {
        ...
        {
            return string.Format("{4}{0:X2}{1:X2}{2:X2}{3:X2}", color.r.ToByte(), color.g.ToByte(), color.b.ToByte(), color.a.ToByte(), leadingHashtag ? "#" : string.Empty);
        }
...
//        }
        ...
        private static readonly Regex HexColorRegex = new Regex("^(?<hashtag>#)?(?<red>[0-9A-Fa-f]{2})(?<green>[0-9A-Fa-f]{2})(?<blue>[0-9A-Fa-f]{2})(?<alpha>[0-9A-Fa-f]{2})?$", RegexOptions.Compiled);
